                        ...
                        return (
                          <div key={item.id} className="border border-gray-200 dark:border-dark-border rounded-lg p-6">
                            ...
                            <div className="flex items-center space-x-3 mb-4">
                              <i className={`fas ${
                                fileName?.toLowerCase().endsWith('.pdf') ? 'fa-file-pdf icon-pdf' :
                                fileName?.toLowerCase().match(/\.(doc|docx)$/) ? 'fa-file-word icon-word' :
                                fileName?.toLowerCase().endsWith('.txt') ? 'fa-file-alt icon-text' :
                                fileName?.toLowerCase().match(/\.(xls|xlsx|csv)$/) ? 'fa-file-excel icon-excel' :
                                fileName?.toLowerCase().match(/\.(ppt|pptx)$/) ? 'fa-file-powerpoint icon-powerpoint' :
                                fileName?.toLowerCase().match(/\.(jpg|jpeg|png|gif|bmp|tiff|svg)$/) ? 'fa-file-image icon-image' :
                                'fa-file icon-text'
                              } text-2xl`}></i>
